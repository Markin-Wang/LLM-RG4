...
class FieldParser:
    ...
    def parse(self, features):
        ...
        last_report = ''
        last_finding = features['last_finding']
        last_impression = features['last_impression']
        if last_finding != 's' and last_impression != 's':
            last_report = last_finding + ' ' + last_impression
        else:
            if last_finding == 's':
                last_report = last_impression
            if last_impression == 's':
                last_report = last_finding

        # to_return['APPA_flag'] = features['APPA_flag']
        last_report = self.clean_report(last_report)
        to_return['last_report'] = last_report
        ...
        # chest x-ray images
        lateral_imagepath = features['lateral_imagepath']
        lateral_image = Image.open(self.args.base_dir + lateral_imagepath)
        lateral_inputs = self.rad_dino_processor(images=lateral_image, return_tensors="pt")
        lateral_images = lateral_inputs.data['pixel_values']
        to_return["lateral_image"] = torch.squeeze(lateral_images, 0)  # image tensor

        last_APPA_imagepath = features['last_APPA_imagepath']
        last_image = Image.open(self.args.base_dir + last_APPA_imagepath)
        last_inputs = self.rad_dino_processor(images=last_image, return_tensors="pt")
        last_images = last_inputs.data['pixel_values']
        to_return["last_image"] = torch.squeeze(last_images, 0)  # image tensor
...
class ParseDataset(data.Dataset):
    def __init__(self, args, split='train'):
        self.args = args
        self.meta = json.load(open(args.mw_annotation, 'r'))
