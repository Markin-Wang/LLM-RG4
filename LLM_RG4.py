...
class LLM_RG4(pl.LightningModule):
    def __init__(self, args):
        ...
        if args.stage_class == 1:
            ...
            if self.args.if_pad == False:
                self.i_proj = nn.Linear(4096,4096)
                self.i_layernorm = nn.LayerNorm(4096)
                self.ii_proj = nn.Linear(4096 * 2,4096)
                self.ii_layernorm = nn.LayerNorm(4096)
                self.it_proj = nn.Linear(4096 * 2,4096)
                self.it_layernorm = nn.LayerNorm(4096)
        ...
        if args.llm_use_lora:
            ...
            print('Loading Vicuna LoRA Done')
        else:
            self.embed_tokens = self.llama_model.get_input_embeddings()
        ...
        if args.visual_delta_file is not None:
            state_dict = \
            torch.load(args.visual_delta_file, map_location=torch.device(f'cuda:{torch.cuda.current_device()}'))['model']
    ...
    def encode_lateral_image(self, images, query_feature):
        outputs = self.visual_encoder(images).last_hidden_state
        outputs = outputs[:,1:,:]
        # vision_query_tokens = self.lateral_visual_query_tokens.expand(images.shape[0], -1, -1)
    ...
    def encode_text(self, text,query_feature):
        ...
        # text_query_tokens = self.text_query_tokens.expand(query_feature.shape[0], -1, -1)
    ...
    def prompt_sn(self, img_embeds, atts_img, samples,sn_token):
        ...
        for b in range(batch_size):
            ...
            # new_p_before_embeds[b,post_pad_len + 1,:]= sn_token
    ...
    def forward(self, samples):
        ...
        if dataset_id == 'sn':
            ...
            if self.args.stage_class != 1 or self.args.if_direct == False:
                new_embeds = ''
                if self.args.if_pad == True:
                    ...
                    new_embeds = self.i_proj(img_embeds)
                    new_embeds = self.i_layernorm(new_embeds)
                sn_token = self.sn_token    # .expand(query_feature.shape[0], -1, -1)
                img_embeds, atts_img = self.prompt_sn(new_embeds, atts_img1, samples,sn_token)

        if dataset_id == 'mn':
            if self.args.whether_notadd:
                img_embeds, atts_img, query_feature = self.encode_APPA_img(image)
                img_embeds, atts_img = self.prompt_sn(img_embeds, atts_img, samples)
            else:
                ...
                if self.args.if_pad == True:
                    ...
                    new_embeds = torch.cat([img_embeds1, lateral_embed], dim=2)
                    new_embeds = self.ii_proj(new_embeds)
                    new_embeds = self.ii_layernorm(new_embeds)
                mn_token = self.mn_token      #.expand(query_feature.shape[0], -1, -1)
                img_embeds, atts_img = self.prompt_sn(new_embeds, atts_img1, samples,mn_token)
        ...
        if dataset_id == 'sw':
            if self.args.whether_notadd:
                ...
                if self.args.if_pad == True:
                    ...
                    new_embeds = torch.cat([img_embeds1,last_report_embed], dim=2)
                    new_embeds = self.it_proj(new_embeds)
                    new_embeds = self.it_layernorm(new_embeds)
                sw_token = self.sw_token         #.expand(query_feature.shape[0], -1, -1)
                img_embeds, atts_img = self.prompt_sn(new_embeds, atts_img1, samples,sw_token)
        ...
        if dataset_id == 'mw':
            if self.args.whether_notadd:
                ...
                new_embeds = self.iit_proj(new_embeds)
                new_embeds = self.iit_layernorm(new_embeds)
                # new_embeds = self.merge_mw(lateral_image,samples,query_feature,img_embeds1)
                mw_token = self.mw_token            #.expand(query_feature.shape[0], -1, -1)
                img_embeds, atts_img = self.prompt_sn(new_embeds, atts_img1, samples,mw_token)
        ...
        gauss_kernel = gauss_kernel.view(1, 1, kernel_size).to(device)
        input_tensor = scores_ori.unsqueeze(1)
        filtered_tensor = F.conv1d(input_tensor, gauss_kernel, padding=(kernel_size // 2))
        filtered_scores = filtered_tensor.squeeze(1)

        scores = ''
        if self.args.loss_mode == 'sentence' or self.args.loss_mode == 'None':
            scores = torch.cat([zero_tensor, scores_ori], dim=1)
        if self.args.loss_mode == 'token':
            scores = torch.cat([zero_tensor, filtered_scores], dim=1)
        ...
        if self.args.loss_mode == 'token':
            shift_scores = torch.where(shift_scores < 0.15, torch.tensor(0.0), torch.tensor(1.0))
        ...
        print('average_loss:' + str(average_loss.item()) + 'key_loss' + str(key_loss.item()))
    ...
    def on_validation_epoch_end(self):
        ...
        chexbert = CheXbert(
            ckpt_dir=self.args.chexbert_ckpt_dir,
            bert_path=self.args.chexbert_bertpath,
            checkpoint_path=self.args.chexbert_checkpoint_path,
            device=device
        ).to(device)
        ...
        val_score = eval_res['Bleu_4']*0.7 + 0.3 * chexbert_f1_MACRO
        if self.trainer.local_rank == 0:
            # if val_score > self.val_score:
            self.save_checkpoint(eval_res,chexbert_f1_MACRO)
    ...
    def configure_optimizers(self):
        ...
        total_steps = self.hparams.max_epochs * 690432 / self.args.batch_size / self.args.accumulate_grad_batches  # all:690432 mw:23849*2 mn:45668*2
